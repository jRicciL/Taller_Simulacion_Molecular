...
def compare_two_distributions(df):
    actives = df.query('actividad == 1')['score']
    inactives = df.query('actividad == 0')['score']
    dk_scores = df

    # Normality test
    print('Normalidad Activos (ShapiroWilks)')
    test = shapiro(actives)
    display(f'> W = {np.round(test[0], 2)} , p-value={np.round(test[1], 4)}')
    print('Normalidad Inactivos (ShapiroWilks)')
    test = shapiro(inactives)
    display(f'> W = {np.round(test[0], 2)} , p-value={np.round(test[1], 4)}')
    # Bartlett
    print('Prueba de Bartlett para Homocedasticidad')
    test = bartlett(actives, inactives)
    display(f'> W = {np.round(test[0], 2)} , p-value={np.round(test[1], 4)}')
    # T-test
    print('t-student para pruebas independientes')
    test = ttest_ind(actives, inactives)
    display(f'> t = {np.round(test[0], 2)} , p-value={np.round(test[1], 4)}')
    # Compute Wilcoxon
    print('Mann-Whitney (no paramétrica)')
    test = mannwhitneyu(actives, inactives)
    display(f'> W = {np.round(test[0], 2)} , p-value={np.round(test[1], 4)}')
    med_actives = actives.median()
    med_inactives = inactives.median()
    
    fig, (box, hist) = plt.subplots(2, 
                            sharex=True, 
                            figsize=(10, 8),
                            gridspec_kw= {"height_ratios": (0.2, 1)})
    
    box.set(title='Distribución de scores Activos/Inactivos')
#     sns.boxplot(x=inactives, ax=box)
    sns.boxplot(x='score', y='actividad', data=dk_scores, ax=box, orient='h')
    box.set(xlabel=None, ylabel=None)
    
    hist = sns.distplot(inactives, 
                      label='Decoys/Inactivos',hist_kws={'linewidth': 0}, norm_hist=True)
    sns.distplot(actives, ax=hist, bins=35,
                 label='Actividad', hist_kws={'linewidth': 0}, norm_hist=True)
    hist.axvline(med_actives, linestyle='--', c='#BD5D19')
    hist.axvline(med_inactives, linestyle='--')
    hist.set(xlabel=f'Scoring', ylabel='Densidad')
